...
get_cc_evol_run <- function(pit_run, reef_type_run, coord_site_run, reunion_bd, run_reef, bati_run) {
  ...
  cc_evol_run <- get_cc_evol(
    pit_region = pit_run, 
    coord_site = coord_site_run,
    fringing   = NULL,
    barrier    = NULL,
    intern     = NULL,
  ...
  cities <- list(
    st_gille   = list(xlim = c(55.2, 55.27), ylim = c(-21.01, -21.125), color = "red"),
    st_leu     = list(xlim = c(55.265, 55.3), ylim = c(-21.133, -21.205), color = "blue"),
    etang_sale = list(xlim = c(55.322, 55.35), ylim = c(-21.26, -21.285), color = "green"),
    st_pierre  = list(xlim = c(55.44, 55.50), ylim = c(-21.33, -21.36), color = "yellow")
    )
  
  
  plot_cities <- setNames(lapply(names(cities), function(x) {
    ...
    get_map_cc_evol(
      ...
      map_reef = run_reef, 
      cc_evol = cc_evol_run, 
      color = c("#0066CC", "#66CCFF"), 
      labels = c("pente externe", "platier récifal"),
      shape = c(15, 15)
      ) +
      ggplot2::geom_sf(data = bati_run, fill = "grey") +
      ggplot2::coord_sf(xlim = town$xlim, ylim = town$ylim) +
      ggplot2::theme(
        panel.background = ggplot2::element_blank(),
        axis.text.x = ggplot2::element_blank(),
        axis.text.y = ggplot2::element_blank(),
        ...
        axis.ticks.y = ggplot2::element_blank(),
        axis.line = ggplot2::element_blank(),
        panel.border = ggplot2::element_rect(color = town$color, fill = NA),
        legend.position = "none"
        )
  ...
  plot_assemble <- cowplot::ggdraw()+
    cowplot::draw_plot(plot_cities$st_gille, x = 0, y = 0, width = 0.5, height = 1) +
    cowplot::draw_plot(plot_cities$st_leu, x = 0.5, y = 0, width = 0.5, height = 1) 
  
   plot_assemble2 <- cowplot::ggdraw()+
    cowplot::draw_plot(plot_cities$etang_sale, x = 0, y = 0.5, width = 0.5, height = 0.5) +
    cowplot::draw_plot(plot_cities$st_pierre, x = 0, y = 0, width = 1, height = 0.5) +
